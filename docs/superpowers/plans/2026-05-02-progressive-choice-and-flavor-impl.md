...
- [ ] **Step 3: 在每个 Act 主入口调用 `推进阶段()`**

`kongjiang_act1.ink` 入口处加 `~ 推进阶段(1)`，`kongjiang_act2.ink` 入口加 `~ 推进阶段(2)`，`kongjiang_act3.ink` 入口加 `~ 推进阶段(3)`。读各文件前 10 行定位到主 knot 起始处。
...
```bash
git add src/_data.ink src/_helpers.ink src/kongjiang_act1.ink src/kongjiang_act2.ink src/kongjiang_act3.ink
...
**关键 ink 细节：**
- 子话题用 `*` 一次性消失（"看过即消失"决策）
- 子话题菜单返回用 `->->` （tunnel 返回，回到调用 `对话_昕` 的场景）
...
- [ ] **Step 5: 编译 + playthrough**（Act1 起进入对话_昕、Act2 起再进一次确认中期话题出现）
...
- [ ] **Step 1: Grep 砚波相关 knot**，可能不止一个。
- [ ] **Step 2: 读相关 knot**
- [ ] **Step 3: 决定 flavor 嵌入点 — 推荐：在 `场景_泡面之争` 完成后或 Act1 新增 `对话_砚波` knot 中**
- [ ] **Step 4: 写两段 flavor：**
...
- [ ] **Step 2: 评估方案（按读取结果选 A/B）**

  - **方案 A（推荐）：** 现有 4 选项中至多有 2 个是常驻基础；保留这 2 个 + 4 个条件支援（`{陪伴_X}` 修饰）。同时显隐让总可见数 ≤4：玩家如果只陪了 1 人，看到 2 基础 + 1 支援 = 3 选项；陪 2 人则 2 基础 + 2 支援 = 4 选项。
  - **方案 B（兜底）：** 把支援做成"专属一屏"——主菜单加一个 `+ {陪伴_X || 陪伴_Y || ...} [盟友支援] -> 支援菜单` 入口，二级菜单展开 4 条件选项。这是渐进式风格但 boss 战要的是扁平紧迫感，**只在方案 A 不可行时用**。
...
跑两个对照 playthrough：
- 喘息夜2 不陪任何人 → boss 阶段 2 看到原 2 基础选项（验证向后兼容）
- 喘息夜2 陪昕 + 志勇 → boss 阶段 2 多 2 个条件支援（验证标记生效）
...
| §5 phase 门槛 | Task 1.1 + 4.3（推进阶段(3)） |
| §6 4 PR 顺序 | Phase 1-4 |
